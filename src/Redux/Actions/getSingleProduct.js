...
export const getSingleProduct = (productId) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`http://www.api.warriordev.tech/api/v1/singleProduct/${productId}`);
...
export const addReview = (formData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(`http://www.api.warriordev.tech/api/v1/Review/new`, formData, config);
...
export const getReviews = (productId) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `http://www.api.warriordev.tech/api/v1/allReviews?productId=${productId}`
    );
